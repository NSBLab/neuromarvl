...
class NeuroMarvl {
    constructor() {
        ...
        */
        this.initUI = () => {
            ...
            */
            ...
            $('#pin').draggable({ containment: '#outer-view-panel' }).on('drag', (event, ...args) => {
                let ui = args[0];
        ...
        */
        ...
        this.batchProcess = (i, numberOfFiles, attributes, matrices, filename = null, filetype = "svg", resolution = { x: 1920, y: 1080 }) => {
        ...
        this.uploadTextFile = (file, fileType) => {
            var reader = new FileReader();
            reader.onload = () => {
        ...
        this.changeFileStatus = (file, status) => {
            ...
            $('#' + file).tooltip('update');
            $('#' + file).tooltip('show');
            setTimeout(function () { $('#' + file).tooltip('hide'); }, 3000);
        };
        this.loadUploadedData = (saveObj, func, source) => {
            var status = {
                coordLoaded: false,
                matrixLoaded: false,
                attrLoaded: false,
        ...
        this.setSelectEdgeKeyBackgroundColor = (color) => {
            if (color.length === 6)
                color = "#" + color;
        ...
        this.setSelectNodeKeyBackgroundColor = (color) => {
            if (color.length === 6)
                color = "#" + color;
        ...
        this.setEdgeDirectionGradient = () => {
            this.saveFileObj.edgeSettings.directionStartColor = $('#input-edge-start-color').val();
            this.saveFileObj.edgeSettings.directionEndColor = $('#input-edge-end-color').val();
            this.validApplicationIDX.forEach(function (i) {
                if (this.applicationsInstances[i])
                    this.applicationsInstances[i].setEdgeDirectionGradient();
            }, this);
        };
        this.setEdgeColorByWeight = () => {
            ...
            this.validApplicationIDX.forEach(function (i) {
                if (this.applicationsInstances[i])
                    this.applicationsInstances[i].setEdgeColorByWeight(config);
            }, this);
        };
        this.setEdgeColorByNode = () => {
            ...
            this.validApplicationIDX.forEach(function (i) {
                if (this.applicationsInstances[i])
                    this.applicationsInstances[i].setEdgeColorByNode();
            }, this);
        };
        this.setEdgeNoColor = () => {
            // save edge color setting 
            ...
            this.validApplicationIDX.forEach(function (i) {
                if (this.applicationsInstances[i])
                    this.applicationsInstances[i].setEdgeNoColor();
            }, this);
        };
        ...
        this.setNodeSizeOrColor = () => {
            ...
            if (sizeOrColor == "node-size") {
                ...
                var scaleMap = d3.scaleLinear().domain([minScale, maxScale]).range([values[0], values[1]]);
                var newScaleArray = scaleArray.map((value) => { return scaleMap(value); });
                this.validApplicationIDX.forEach(function (i) {
                    if (this.applicationsInstances[i])
                        this.applicationsInstances[i].setNodeSize(newScaleArray);
                }, this);
                ...
                if ((this.referenceDataSet.attributes.info[attribute] != null) && (this.referenceDataSet.attributes.info[attribute].isDiscrete && !nodeColorMode)) {
                    ...
                    this.validApplicationIDX.forEach(function (i) {
                        if (this.applicationsInstances[i])
                            this.applicationsInstances[i].setNodeColorDiscrete(attribute, keyArray, colorArray);
                    }, this);
                }
                else {
                    let minColor = $('#input-min-color').val();
                    let maxColor = $('#input-max-color').val();
                    //#et minColor = "#000000";
                    //let maxColor = "#000000";
                    this.validApplicationIDX.forEach(function (i) {
                        if (this.applicationsInstances[i])
                            this.applicationsInstances[i].setNodeColor(attribute, minColor, maxColor);
                    }, this);
                ...
                this.validApplicationIDX.forEach(function (i) {
                    if (this.applicationsInstances[i])
                        this.applicationsInstances[i].setNodeDefaultSizeColor();
                }, this);
                // Edge will also need updating if they are set to "node"
                if (this.commonData.edgeColorMode === "node") {
                    this.setEdgeColorByNode();
        ...
        this.saveSettings = (filename) => {
            ...
            for (var i = 0; i < 4; i++) {
                ...
                if (this.applicationsInstances[i])
                    this.applicationsInstances[i].save(saveAppObject);
            }
            /*
        ...
        this.exportSVG = (viewport, fileType, resolution, filename, callback = null) => {
            var documents = [window.document], SVGSources = [];
            // loop through all active app
            if (!this.applicationsInstances[viewport])
                return;
            ...
            this.applicationsInstances[viewport].resize(resolution.x, resolution.y, 'screenshotzoomstart');
            //console.log(canvas);
            ...
            requestAnimationFrame(() => {
                ...
                requestAnimationFrame(() => {
                    this.applicationsInstances[viewport].resize(this.applicationsInstances[viewport].jDiv.width(), this.applicationsInstances[viewport].jDiv.height(), 'screenshotzoomend');
                    if (prevsvgtransform != null)
        ...
        this.downloadSVG = (source, filename) => {
            if (filename == null)
        ...
        this.downloadSVGImage = (source, filename) => {
            if (filename == null)
                filename = window.document.title.replace(/[^a-z0-9]/gi, '-').toLowerCase();
        ...
        this.getNodeScaleArray = (attribute) => {
        ...
        this.setupColorPickerDiscrete = (attribute) => {
            ...
            var d3ColorSelector = d3.scaleOrdinal(d3.schemeCategory10); //cccc
            var uniqueColors = uniqueKeys.map((group) => { return d3ColorSelector(group); });
            ...
            $("#input-node-color").val(uniqueColors[0]);
        };
        // Find which view is currently located under the mouse
        this.getViewUnderMouse = (x, y) => {
            let $view = $(TL_VIEW);
        ...
        this.getActiveTargetUnderMouse = (x, y) => {
            let view = this.getViewUnderMouse(x, y);
            return this.viewToId(view);
        };
        this.setNodeColorInContextMenu = (color) => {
            if (color.length === 6)
                color = "#" + color;
        ...
        this.setBrainMode = (brainMode, view) => {
            switch (view) {
                case TL_VIEW:
            ...
            this.loadBrainModel(model, object => {
                ...
                let makeBrain = () => {
                    this.applicationsInstances[viewTypeId] = new Brain3DApp({
                        ...
                        brainSurfaceMode
                    }, this.commonData, this.input.newTarget(viewTypeId), this.saveFileObj);
        ...
        this.setEdgeDirection = () => {
            ...
            this.validApplicationIDX.forEach(function (i) {
                if (this.applicationsInstances[i])
                    this.applicationsInstances[i].setEdgeDirection(value);
            }, this);
        };
        this.setEdgeColor = () => {
            ...
            if (value === "none") {
                ...
                if (this.referenceDataSet.info.edgeWeight.type === "continuous" || this.commonData.edgeForceContinuous) {
                    ...
                    var distinctColors = distinctValues.map((group) => { return d3ColorSelector(group); });
        ...
        this.setViewCrossroads = (x, y) => {
            ...
            if (this.applicationsInstances[0])
                this.applicationsInstances[0].resize(lw, th, 'resize');
            if (this.applicationsInstances[1])
                this.applicationsInstances[1].resize(rw, th, 'resize');
            if (this.applicationsInstances[2])
                this.applicationsInstances[2].resize(lw, bh, 'resize');
            if (this.applicationsInstances[3])
                this.applicationsInstances[3].resize(rw, bh, 'resize');
        };
        ...
        this.initSurfaceSettings = () => {
            ...
            if (this.saveFileObj.surfaceSettings.color) {
                $("#input-surface-color").val(this.saveFileObj.surfaceSettings.color);
                this.setBrainSurfaceColor($("#input-surface-color").val());
            }
        };
        /**
        ...
        */
        this.initEdgeSizeAndColor = () => {
            ...
            if (this.saveFileObj.edgeSettings.directionMode != null) {
                ...
                if (this.saveFileObj.edgeSettings.directionStartColor != null) {
                    $('#input-edge-start-color').val(this.saveFileObj.edgeSettings.directionStartColor);
                    $('#input-edge-end-color').val(this.saveFileObj.edgeSettings.directionEndColor);
                }
            ...
            if (this.saveFileObj.edgeSettings.colorBy === "none") {
                ...
                if (this.saveFileObj.edgeSettings.weight.type === "discrete") {
                    ...
                    $('#input-edge-min-color').val(setting.minColor);
                    $('#input-edge-max-color').val(setting.maxColor);
                }
                    ...
                    for (var i = 0; i < setting.numCategory; i++) {
                        $('#input-edge-discretized-' + i + '-color').val(setting.colorArray[i]);
                    }
                }
                else {
            ...
            if (this.saveFileObj.edgeSettings.edgeColorByNodeTransition != null) {
                if (this.saveFileObj.edgeSettings.edgeColorByNodeTransitionColor) {
                    $("#input-edge-transitional-color").val(this.saveFileObj.edgeSettings.edgeColorByNodeTransitionColor);
        ...
        this.initNodeColor = () => {
            if (this.saveFileObj.nodeSettings.nodeColorAttribute) {
                ...
                if (this.referenceDataSet.attributes.info[this.saveFileObj.nodeSettings.nodeColorAttribute].isDiscrete) {
                    ...
                    $("#input-node-color").val(this.saveFileObj.nodeSettings.nodeColorDiscrete[0]);
                }
                else {
                    ...
                    $("#input-min-color").val(tmpMin);
                    $("#input-max-color").val(tmpMax);
        ...
        this.loadBrainModel = (model, callback) => {
        ...
        this.setBrainSurfaceColor = (color) => {
            this.saveFileObj.surfaceSettings.color = color;
            this.validApplicationIDX.forEach(function (i) {
                if (this.applicationsInstances[i])
                    this.applicationsInstances[i].setSurfaceColor(color);
            }, this);
        };
        this.setEdgeTransitionColor = (color) => {
            this.saveFileObj.edgeSettings.edgeColorByNodeTransitionColor = color;
            this.validApplicationIDX.forEach(function (i) {
                if (this.applicationsInstances[i])
                    this.applicationsInstances[i].setEdgeTransitionColor(color);
            }, this);
        };
        this.setUseTransitionColor = (useColor) => {
            this.saveFileObj.edgeSettings.edgeColorByNodeTransition = useColor;
            this.validApplicationIDX.forEach(function (i) {
                if (this.applicationsInstances[i])
                    this.applicationsInstances[i].setUseTransitionColor(useColor);
            }, this);
        };
        // Load the similarity matrix for the specified dataSet
        //TODO: Move into DataSet class
        this.loadSimilarityMatrix = (file, dataSet, callback) => {
            var reader = new FileReader();
            reader.onload = () => {
        ...
        this.parseSimilarityMatrix = (text, dataSet) => {
            //TODO: Move into DataSet class
            var lines = text.replace(/\t|\,/g, ' ').trim().split(/\r\n|\r|\n/g).map(s => s.trim());
            var simMatrix = [];
        ...
        //TODO: Move into DataSet class
        this.loadAttributes = (file, dataSet, callback) => {
            var reader = new FileReader();
            reader.onload = () => {
        ...
        this.setupCrossFilter = (attrs) => {
            ...
            for (var j = 0; j < attrs.columnNames.length; ++j) {
                ...
                chart
                    ...
                    .dimension(dim)
                    .group(group)
                    .x(d3.scaleLinear().domain([minValue - offset, maxValue + offset]))
        ...
        */
        this.initListeners = () => {
            $(document).on("keyup", (e => {
                if (e.code == 'Escape')
                    this.toggleSplashPage(); // esc
            }));
            // Set up color pickers
            // input is when the user clicks on a different colour
            // change is when the color choice is confirmed, or set with .val()
            $("#input-node-color").on("input change", e => {
                this.setSelectNodeKeyBackgroundColor($(e.target).val());
                this.setNodeSizeOrColor();
            });
            //console.log((<any>$("#input-surface-color")).colorpicker());
            //$("#input-surface-color").on('colorpickerChange', e => { console.log(e); this.setBrainSurfaceColor((<any>$(e.target)).colorpicker('getValue')); });
            $("#input-surface-color").on('input change', e => { this.setBrainSurfaceColor($(e.target).val()); });
            $("#input-min-color").on("input change", e => this.setNodeSizeOrColor());
            ...
            $("#input-edge-color").on("input change", e => {
                this.setSelectEdgeKeyBackgroundColor($(e.target).val());
                this.setEdgeColorByWeight();
            });
            //$("#input-context-menu-node-color").on("change", e => this.setNodeColorInContextMenu($(<any>e).val()));
            $("#input-context-menu-node-color").on("input change", e => this.setNodeColorInContextMenu($(e.target).val()));
            $("#input-edge-transitional-color").on("input change", e => this.setEdgeTransitionColor($(e.target).val()));
            $('#button-select-matrix').on("click", () => $("#select-matrix")[0].click());
            $('#select-matrix').on('change', () => {
                // Change the button name according to the file name
                let file = $('#select-matrix').get(0).files[0];
            ...
            $('#button-save-app').button().on("click", () => {
                ...
                for (var i = 0; i < 4; i++) {
                    ...
                    if (this.applicationsInstances[i])
                        this.applicationsInstances[i].save(app);
                }
            ...
            $("#checkbox-color-transitional-edges").on("change", () => {
                ...
                this.validApplicationIDX.forEach(function (i) {
                    if (this.applicationsInstances[i])
                        this.applicationsInstances[i].circularGraph.update();
                }, this);
            });
            ...
            $('#select-node-key').on('change', () => {
                ...
                for (var i = 0; i < keySelection.length; i++) {
                    if (keySelection.options[i].value == key) {
                        var color = keySelection.options[i].style.backgroundColor;
                        var hex = this.colorToHex(color);
                        console.log("setting color of the color picker to " + hex);
                        $("#input-node-color").val(hex);
                        break;
                    }
                }
            });
        ...
        this.validApplicationIDX = Array(4).fill(0).map((element, index) => index);
    }
