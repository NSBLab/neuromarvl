...
function getActiveTargetUnderMouse(x, y) {
    var id = -1;
...
function setNodeColorInContextMenu(color) {
...
function setCircularBar1Color(color) {
    if (apps[input.activeTarget]) {
        apps[input.activeTarget].setCircularBarColor(1, '#' + color);
    }
}

function setCircularBar2Color(color) {
    if (apps[input.activeTarget]) {
...
<<<<<<< Updated upstream
        ...
        if (apps[0])
            apps[0].highlightSelectedNodes(idArray);
        if (apps[1])
            apps[1].highlightSelectedNodes(idArray);
        if (apps[2])
            apps[2].highlightSelectedNodes(idArray);
        if (apps[3])
            apps[3].highlightSelectedNodes(idArray);
    }
=======
=======
>>>>>>> d2c0c4a93bc9257fcb4df0dd36767a98f8c8a07a
...
<<<<<<< HEAD
        ...
        this.parseLabels = function (text) {
            ...
            console.log(_this.referenceDataSet.brainLabels);
...
function brainIconDraggableEvent(model, view) {
    resetBrain3D();
    ...
    save.saveApps[appID].surfaceModel = model;
    save.saveApps[appID].view = view;
}
...
$('#dataset2-icon-front').draggable({
    ...
    stop: function (event) {
        ...
        switch (getViewUnderMouse(event.pageX, event.pageY)) {
            case tl_view:
                if (apps[0])
                    apps[0].setDataSet(dataSets[1]);
                break;
            case tr_view:
                if (apps[1])
                    apps[1].setDataSet(dataSets[1]);
                break;
            case bl_view:
                if (apps[2])
                    apps[2].setDataSet(dataSets[1]);
                break;
            case br_view:
                if (apps[3])
                    apps[3].setDataSet(dataSets[1]);
                break;
        }
    }
});
...
function resetIcon(object, location) {
    return function () {
...
var apps = Array(new DummyApp(), new DummyApp(), new DummyApp(), new DummyApp());
...
function setEdgeSize() {
    ...
    if (apps[0])
        apps[0].setEdgeSize(edgeSize);
    if (apps[1])
        apps[1].setEdgeSize(edgeSize);
    if (apps[2])
        apps[2].setEdgeSize(edgeSize);
    if (apps[3])
        apps[3].setEdgeSize(edgeSize);
}
